...
// API Configuration - UPDATED WITH LIVE URL
const API_CONFIG = {
    baseURL: 'https://api.sabjihaat.in/api',
    endpoints: {
        products: '/products',
        orders: '/orders',
        settings: '/settings',
...
const apiService = {
    async request(endpoint, options = {}) {
        ...
        if (adminToken && !endpoint.includes('/auth/login')) {
    ...
    async getProducts(category = null) {
        const url = category ? `${API_CONFIG.endpoints.products}/category/${category}` : API_CONFIG.endpoints.products;
    ...
    async validatePincode(pincode) {
        return this.request(`${API_CONFIG.endpoints.pincodes}/validate/${pincode}`);
    },
...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        if (savedToken) {
            try {
                const verification = await apiService.verifyToken(savedToken);
...
async function loadSettingsFromBackend() {
    try {
        ...
        if (response.success) {
            ...
            BUSINESS_INFO.deliveryFee = settings.deliveryFee;
            BUSINESS_INFO.freeDeliveryMin = settings.freeDeliveryMin;
            BUSINESS_INFO.servicePincodes = settings.servicePincodes;
            BUSINESS_INFO.pincodeCharges = settings.pincodeCharges;
        } else {
            throw new Error(response.error);
        }
    } catch (error) {
        ...
        BUSINESS_INFO.deliveryFee = 20;
        BUSINESS_INFO.freeDeliveryMin = 999;
        BUSINESS_INFO.servicePincodes = ["700047", "700045", "700075", "700094", "700084", "700092", "700040", "700068", "700095", "700032", "700031"];
        BUSINESS_INFO.pincodeCharges = {
...
function loadDemoProducts() {
    PRODUCTS = {
        indian: [
            { _id: '1', name: "Fresh Lemon", price: 5, image: "https://images.unsplash.com/photo-1642372849451-18113b4c5cd2?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTJ8fGdyZWVuJTIwbGVtb258ZW58MHx8MHx8fDA%3D&auto=format&fit=crop&w=600&q=60", unit: "piece", stock: 50, category: "indian" },
            { _id: '2', name: "Organic Tomato", price: 40, image: "https://images.unsplash.com/photo-1546470427-e212b7d31055?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8dG9tYXRvfGVufDB8fDB8fHww&auto=format&fit=crop&w=600&q=60", unit: "kg", stock: 100, category: "indian" },
            { _id: '3', name: "Fresh Potato", price: 30, image: "https://images.unsplash.com/photo-1518977676601-b53f82aba655?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8cG90YXRvfGVufDB8fDB8fHww&auto=format&fit=crop&w=600&q=60", unit: "kg", stock: 80, category: "indian" },
        ],
        exotic: [
            { _id: '4', name: "Fresh Broccoli", price: 80, image: "https://images.unsplash.com/photo-1459411552884-841db9b3cc2a?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8YnJvY2NvbGl8ZW58MHx8MHx8fDA%3D&auto=format&fit=crop&w=600&q=60", unit: "piece", stock: 20, category: "exotic" }
        ],
        leafy: [
            { _id: '5', name: "Green Spinach", price: 25, image: "https://images.unsplash.com/photo-1576045057995-568f588f82fb?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8c3BpbmFjaHxlbnwwfHwwfHx8MA%3D%3D&auto=format&fit=crop&w=600&q=60", unit: "bunch", stock: 30, category: "leafy" }
        ],
        others: [
            { _id: '6', name: "Fresh Paneer", price: 120, image: "https://images.unsplash.com/photo-1598488000051-c7b1a7a77936?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8cGFuZWVyfGVufDB8fDB8fHww&auto=format&fit=crop&w=600&q=60", unit: "packet", stock: 25, category: "others" }
        ]
    };
}
...
function updateOfferBanner() {
    ...
    offerBanner.innerHTML = `
        🚚 Free Delivery on orders above ₹${freeDeliveryMin} within service area!  
        <div class="slider-item">📞 Call/WhatsApp: +919051410591</div>
    `;
}
...
function loadProducts(category) {
        ...
        productCard.innerHTML = `
            ...
            <div class="product-info">
                ...
                <div class="quantity-selector">
                    <div class="quantity-options" id="quantity-options-${product._id}">
                        ${quantityOptions.map(option => `
                            <div class="quantity-option" data-value="${option.value}" data-product="${product._id}">${option.label}</div>
                        `).join('')}
                    </div>
                    <div class="custom-quantity">
                        <input type="number" class="custom-quantity-input" id="custom-quantity-${product._id}" placeholder="Custom" min="0.1" step="0.1" oninput="validateNumberInput(this)" ${isOutOfStock ? 'disabled' : ''}>
                        <span class="custom-quantity-unit">${product.unit}</span>
                    </div>
                </div>
                <button class="add-to-cart" onclick="addToCart('${product._id}')" ${isOutOfStock ? 'disabled' : ''}>${isOutOfStock ? 'Out of Stock' : 'Add to Cart'}</button>
            </div>
        `;
    ...
    products.forEach(product => {
        setupQuantitySelectors(product._id);
    });
}
...
async function addToCart(productId) {
    ...
    let product = null;
    for (const category in PRODUCTS) {
        product = PRODUCTS[category].find(p => p._id === productId);
        if (product) break;
    }
...
async function validatePincode(pincode) {
    if (pincode.length === 6) {
        try {
            const response = await apiService.validatePincode(pincode);
            if (response.success && response.data.isValid) {
                BUSINESS_INFO.deliveryFee = response.data.deliveryFee;
                showNotification('Delivery available in your area', 'success');
                renderCartItems(); // Update cart with new delivery fee
                return true;
            } else {
...
async function processCheckout(name, phone, address, pincode) {
    try {
        ...
        message += `*Order #:* ${orderResponse.data.orderNumber}\n`;
            ...
            PRODUCTS[category].forEach(product => {
                ...
                productItem.innerHTML = `
                    ...
                    <div class="admin-product-actions">
                        <button class="admin-btn edit-btn" onclick="editProduct('${product._id}')">Edit</button>
                        <button class="admin-btn delete-btn" onclick="deleteProduct('${product._id}')">Delete</button>
                    </div>
                `;
...
async function updateProduct() {
    try {
        ...
        const response = await apiService.updateProduct(editingProductId, productData);
...
async function loadPincodes() {
    try {
        const response = await apiService.getPincodes();
        
        if (response.success) {
            const pincodesData = response.data;
        ...
        showNotification('Failed to load pincodes: ' + error.message, 'error');
    }
}
...
async function addPincode() {
    try {
        ...
        // Update local data
        const updatedServicePincodes = [...BUSINESS_INFO.servicePincodes, newPincode];
        const updatedPincodeCharges = { ...BUSINESS_INFO.pincodeCharges, [newPincode]: newCharge };
        
        // Save to backend
        const response = await apiService.updatePincodes({
            servicePincodes: updatedServicePincodes,
            pincodeCharges: updatedPincodeCharges
        });
        ...
        showNotification('Failed to add pincode: ' + error.message, 'error');
    }
}

async function updatePincodeCharge(pincode, charge) {
    try {
        const updatedPincodeCharges = { ...BUSINESS_INFO.pincodeCharges, [pincode]: parseInt(charge) };
        ...
        if (response.success) {
            BUSINESS_INFO.pincodeCharges = updatedPincodeCharges;
            showNotification('Delivery charge updated');
        } else {
            throw new Error(response.error);
        }
    } catch (error) {
        showNotification('Failed to update delivery charge: ' + error.message, 'error');
    }
}
...
async function removePincode(pincode) {
    if (confirm('Are you sure you want to remove this pincode?')) {
        try {
            const updatedServicePincodes = BUSINESS_INFO.servicePincodes.filter(p => p !== pincode);
            const updatedPincodeCharges = { ...BUSINESS_INFO.pincodeCharges };
            delete updatedPincodeCharges[pincode];
            ...
            if (response.success) {
                BUSINESS_INFO.servicePincodes = updatedServicePincodes;
                BUSINESS_INFO.pincodeCharges = updatedPincodeCharges;
            ...
            showNotification('Failed to remove pincode: ' + error.message, 'error');
        }
    }
}
        ...
        PRODUCTS[category].forEach(product => {
            ...
            stockItem.innerHTML = `
                ...
                <div class="stock-actions">
                    ...
                    <input type="number" class="stock-input" value="${product.stock}" onchange="updateStock('${product._id}', this.value)">
                    <button class="admin-btn save-btn" onclick="updateStock('${product._id}', document.querySelector('#stockList input[value=\"${product.stock}\"]').value)">Update</button>
                </div>
            `;
            ...
            if (product.name.toLowerCase().includes(searchTerm)) {
                ...
                stockItem.innerHTML = `
                    ...
                    <div class="stock-actions">
                        ...
                        <input type="number" class="stock-input" value="${product.stock}" onchange="updateStock('${product._id}', this.value)">
                        <button class="admin-btn save-btn" onclick="updateStock('${product._id}', this.previousElementSibling.value)">Update</button>
                    </div>
                `;
                stockList.appendChild(stockItem);
            }
...
async function updateStock(productId, newStock) {
    try {
        ...
        const response = await apiService.updateProduct(productId, { stock: stockValue });
...
// Add logout button to admin panel (update your HTML to include this)
// Add this button in your admin panel HTML:
// <button class="admin-btn logout-btn" onclick="adminLogout()" style="background: #e74c3c; margin-top: 20px;">Logout</button>
